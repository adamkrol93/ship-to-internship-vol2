...
function show_link() {
    echo -e "\n#############################################\n"
    echo "Congratulations, you successfully pass phase 2"
    echo -e "\n#############################################\n"
}
...
for suite in "$MYDIR/test-vectors/phase2/"**; do
    cat "$suite/title.txt"
    ...
    for FIN in "$suite"/*_in; do
        ...
        echo -e "\n\nRunning Main...\n"
        #java -cp target/*.jar -cp target/lib/*.jar idemia.internship.tictactoe.cli.Main $ARGS < "$FIN" > "$FOUT"
