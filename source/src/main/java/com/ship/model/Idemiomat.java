...
import static com.ship.model.Size.MAX_POSSIBLE_FIT;
...
public class Idemiomat {
    ...
    public List<Package> take(String taker) {
        return ofNullable(this.ownerPackages.get(taker))
                .map(takerPackages -> takerPackages.stream()
                        .flatMap(i -> {
                            Cubicle cubicleOfInterest = this.cubicles.get(i);
                            this.cubicles.set(i, cubicleOfInterest.takePackages());
                            return cubicleOfInterest.getPackages().stream();
                        })
                        .sorted(Comparator.comparing(Package::getSize))
                        .collect(Collectors.toList()))
                .orElse(new ArrayList<>());
    }

    public long getOpenCount() {
        return this.cubicles.stream().filter(Cubicle::isFree).count();
    }
}
