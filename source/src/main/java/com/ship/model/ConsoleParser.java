...
public class ConsoleParser {
    ...
    private static Optional<Stream<Package>> parsePackage(String s) {
        ...
        if (splitted.length == 1) {
            return Optional.of(Stream.of(new Package(Size.S, splitted[0])));
        } else {
            if (splitted[0].length() == 0) {
                return Optional.of(Stream.of(new Package(Size.S, splitted[1])));
            }
    ...
    public static List<String> parseTaker(String line) {
        return Arrays.stream(line.replace("T", "").split(";"))
                .map(s -> s.replace("@", ""))
                .collect(Collectors.toList());
    }
}
